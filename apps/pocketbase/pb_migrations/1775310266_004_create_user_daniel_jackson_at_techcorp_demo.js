...
migrate((app) => {
  ...
  record.set("email", "daniel.jackson@techcorp.demo");
  record.setPassword("DemoPass123!");
  record.set("first_name", "Daniel");
  record.set("last_name", "Jackson");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "daniel.jackson@techcorp.demo");
    app.delete(record);
  } catch (e) {
