...
migrate((app) => {
  ...
  record.set("email", "emp3@techcorp.demo");
  record.setPassword("Demo123!");
  record.set("first_name", "Henry");
  record.set("last_name", "Moore");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "emp3@techcorp.demo");
    app.delete(record);
  } catch (e) {
