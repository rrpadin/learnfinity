...
migrate((app) => {
  ...
  record.set("email", "manager1@techcorp.demo");
  record.setPassword("Demo123!");
  record.set("first_name", "Carol");
  record.set("last_name", "Williams");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "manager1@techcorp.demo");
    app.delete(record);
  } catch (e) {
