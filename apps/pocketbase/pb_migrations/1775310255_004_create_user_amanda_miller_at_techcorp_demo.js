...
migrate((app) => {
  ...
  record.set("email", "amanda.miller@techcorp.demo");
  record.setPassword("DemoPass123!");
  record.set("first_name", "Amanda");
  record.set("last_name", "Miller");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "amanda.miller@techcorp.demo");
    app.delete(record);
  } catch (e) {
