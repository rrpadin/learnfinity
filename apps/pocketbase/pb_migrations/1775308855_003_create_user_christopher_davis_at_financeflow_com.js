...
migrate((app) => {
  ...
  record.set("email", "christopher.davis@financeflow.com");
  record.setPassword("FinanceFlow2024!");
  record.set("name", "Christopher Davis");
  record.set("role", "user");
  record.set("is_admin", false);
  record.set("job_title", "Accountant");
  record.set("department", "Finance");
  record.set("hire_date", "2023-03-20");
  record.set("employment_type", "full_time");
  record.set("phone", "+1-555-0204");
  record.set("location", "Chicago, IL");
  record.set("bio", "General ledger and reconciliation");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "christopher.davis@financeflow.com");
    app.delete(record);
  } catch (e) {
