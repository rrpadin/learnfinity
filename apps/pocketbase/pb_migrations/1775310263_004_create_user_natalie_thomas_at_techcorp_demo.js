...
migrate((app) => {
  ...
  record.set("email", "natalie.thomas@techcorp.demo");
  record.setPassword("DemoPass123!");
  record.set("first_name", "Natalie");
  record.set("last_name", "Thomas");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "natalie.thomas@techcorp.demo");
    app.delete(record);
  } catch (e) {
