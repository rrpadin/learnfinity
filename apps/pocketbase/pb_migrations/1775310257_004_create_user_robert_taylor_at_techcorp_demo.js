...
migrate((app) => {
  ...
  record.set("email", "robert.taylor@techcorp.demo");
  record.setPassword("DemoPass123!");
  record.set("first_name", "Robert");
  record.set("last_name", "Taylor");
  ...
  record.set("job_title", "Recruiter");
  record.set("department", "Human Resources");
  try {
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "robert.taylor@techcorp.demo");
    app.delete(record);
  } catch (e) {
